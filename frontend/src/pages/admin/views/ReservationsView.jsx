                ...
                <tbody className="divide-y divide-luxury-black/5">
                    {reservas
                        .filter(res => {
                            ...
                            const matchesStatus =
                                reservaFilter === 'Todas' ||
                                (reservaFilter === 'Confirmadas' && res.estado === 'Confirmada') ||
                                (reservaFilter === 'Pendientes' && res.estado === 'Pendiente') ||
                                (reservaFilter === 'Canceladas' && res.estado === 'Cancelada');
                        ...
                        <tr key={res.id} className="group hover:bg-luxury-black transition-all duration-700">
                            <td className="py-12 px-8 text-2xl font-serif text-luxury-black opacity-10 group-hover:opacity-100 transition-opacity">
                                #{res.id}
                            </td>

                            <td className="py-12 px-8">
                                <p className="text-[12px] uppercase tracking-[0.1em] font-bold text-luxury-black mb-1 font-serif group-hover:text-white">
                                    {res.cliente || "Cliente"}
                                </p>
                                <span className="text-[9px] uppercase tracking-widest text-luxury-gray-mid font-bold group-hover:text-luxury-gray-light">
                                    {res.invitados || 0} Invitados
                                </span>
                            </td>

                            <td className="py-12 px-8 text-sm font-light text-luxury-gray-mid group-hover:text-luxury-gray-light/60">
                                {res.fecha}
                            </td>

                            <td className="py-12 px-8 text-[10px] uppercase tracking-[0.3em] font-bold text-luxury-gray-dark group-hover:text-white">
                                {res.paquete}
                            </td>
                            ...
                            <td className="py-12 px-8">
                                <span className={`text-[9px] uppercase tracking-[0.4em] font-black px-5 py-2 transition-all duration-500 rounded-lg ${
                                    res.estado === 'Confirmada'
                                        ? 'bg-luxury-black text-luxury-white group-hover:bg-white group-hover:text-luxury-black'
                                        : res.estado === 'Pendiente'
                                        ? 'bg-luxury-white border border-luxury-black/10 text-luxury-gray-mid group-hover:bg-white/10 group-hover:text-white'
                            ...
                            <td className="py-12 px-8 text-right text-3xl font-serif font-light tracking-tighter text-luxury-black group-hover:text-white">
                                {res.total}
                            </td>
