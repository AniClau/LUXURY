...
const DashboardView = ({ reservas }) => {
    ...
    // Evita errores si reservas aún no carga
    const listaReservas = reservas || [];

    // Cálculos automáticos
    const totalReservas = listaReservas.length;

    const reservasPendientes = listaReservas.filter(
        r => r.estado === 'Pendiente'
    ).length;

    const ingresosEstimados = listaReservas.reduce((total, r) => {
        const monto = Number((r.total || "0").replace('$','').replace(',',''));
        return total + monto;
    }, 0);
        ...
        <div className="space-y-24 animate-in fade-in slide-in-from-bottom-5 duration-1000">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                {[
                    ...
                    { label: 'Ingresos Estimados', val: `$${ingresosEstimados.toLocaleString()}`, icon: TrendingUp, color: 'text-luxury-black' },
                ].map((stat, i) => (
                ...
                <div className="overflow-x-auto relative z-10">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-luxury-black/5">
                            {listaReservas.slice(0, 5).map((res) => (
